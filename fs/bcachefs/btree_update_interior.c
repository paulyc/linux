...
void bch2_btree_update_add_new_node(struct btree_update *as, struct btree *b)
{
	...
	BUG_ON(bch2_keylist_u64s(&as->new_keys) + b->key.k.u64s >
	       ARRAY_SIZE(as->_new_keys));
	bch2_keylist_add(&as->new_keys, &b->key);
}
						     ...
						     struct btree *b)
{
	...
	BUG_ON(bch2_keylist_u64s(&as->old_keys) + b->key.k.u64s >
	       ARRAY_SIZE(as->_old_keys));
	bch2_keylist_add(&as->old_keys, &b->key);
}
